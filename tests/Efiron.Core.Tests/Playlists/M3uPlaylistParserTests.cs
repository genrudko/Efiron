...
namespace Efiron.Core.Tests.Playlists;
...
public sealed class M3uPlaylistParserTests
{
    ...
    public void Parse_accepts_bom_crlf_attributes_commas_groups_and_catchup_metadata()
    {
        ...
        var playlistUri = new Uri("https://provider.example/account/playlist.m3u");

        var result = _parser.Parse(content, playlistUri);

        Assert.Equal(2, result.Channels.Count);
        Assert.Equal("https://epg.example/guide.xml.gz", result.HeaderAttributes["url-tvg"]);

        var news = result.Channels[0];
        Assert.Equal("News One, HD", news.Name);
        Assert.Equal("news.one", news.TvgId);
        Assert.Equal("News", news.GroupName);
        Assert.Equal("default", news.Attributes["catchup"]);
        Assert.Equal("7", news.Attributes["catchup-days"]);
        Assert.Equal(new Uri("https://provider.example/account/logos/news.png"), news.LogoUri);

        var sport = result.Channels[1];
        Assert.Equal("Sport", sport.GroupName);
    ...
    public void Parse_assigns_unique_ids_to_duplicate_channel_identities()
    {
        ...
        Assert.Equal(2, result.Channels.Count);
        Assert.NotEqual(result.Channels[0].StableId, result.Channels[1].StableId);
    }
    ...
    public void Parse_preserves_player_directives_and_inline_url_options()
    {
        ...
        Assert.Equal("Efiron Test", channel.Directives["extvlcopt:http-user-agent"]);
        Assert.Equal("hls", channel.Directives["kodiprop:inputstream.adaptive.manifest_type"]);
        Assert.Equal("Referer=https://provider.example/", channel.Directives["url-options"]);
    }
}
