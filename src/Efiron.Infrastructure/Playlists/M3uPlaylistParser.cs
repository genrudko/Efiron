...
namespace Efiron.Infrastructure.Playlists;
...
public sealed class M3uPlaylistParser : IPlaylistParser
{
    ...
    public PlaylistDocument Parse(string content, Uri? sourceUri = null)
    {
        ArgumentNullException.ThrowIfNull(content);

        var lines = NormalizeLines(content);
        if (lines.Any(static line => line.TrimStart().StartsWith(
                "#EXT-X-",
        ...
        for (var index = 0; index < lines.Length; index++)
        {
            ...
            if (line.StartsWith("#EXTM3U", StringComparison.OrdinalIgnoreCase))
            {
                ...
                MergeAttributes(
                    headerAttributes,
                    ParseAttributes(line["#EXTM3U".Length..]));
                continue;
            }
            ...
            if (!TryResolveStreamUri(
                    line,
                    sourceUri,
                    out var streamUri,
                    out var inlineOptions))
            {
                warnings.Add(new PlaylistParseWarning(
                    lineNumber,
            ...
            var programmeGuideId = NullIfWhiteSpace(
                GetAttribute(pending.Attributes, "tvg-id"));
            var providerName = NullIfWhiteSpace(
                GetAttribute(pending.Attributes, "tvg-name"));
            var name = NullIfWhiteSpace(pending.DisplayName) ??
                providerName ??
                $"Channel {channels.Count + 1}";
            var category = NullIfWhiteSpace(pending.ExplicitGroup) ??
    ...
    private static PendingEntry ParseExtInf(string line, int lineNumber)
    {
        ...
        var displayName = commaIndex >= 0
            ? payload[(commaIndex + 1)..].Trim()
            : string.Empty;
        ...
        out string? inlineOptions)
    {
        ...
        var uriText = optionSeparator >= 0
            ? sourceLine[..optionSeparator].Trim()
            : sourceLine.Trim();
        inlineOptions = optionSeparator >= 0
            ? sourceLine[(optionSeparator + 1)..].Trim()
            : null;
        ...
        return sourceUri is not null &&
            Uri.TryCreate(sourceUri, uriText, out streamUri);
    }

    private static Uri? ResolveOptionalUri(string? value, Uri? sourceUri)
    {
        if (value is null)
        {
            return null;
        ...
        return sourceUri is not null &&
            Uri.TryCreate(sourceUri, value, out var relativeUri)
            ? relativeUri
            : null;
    }
    ...
    {
        var identity = programmeGuideId is not null
            ? $"tvg-id:{NormalizeIdentityPart(programmeGuideId)}"
            : providerName is not null
                ? $"tvg-name:{NormalizeIdentityPart(providerName)}|group:{NormalizeIdentityPart(category)}"
                : $"name:{NormalizeIdentityPart(displayName)}|group:{NormalizeIdentityPart(category)}";
        ...
        Dictionary<string, string> attributes)
    {
        ...
        public Dictionary<string, string> Directives { get; } = new(
            StringComparer.OrdinalIgnoreCase);
